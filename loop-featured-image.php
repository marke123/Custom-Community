...
if ($list_post_query->have_posts()) : while ($list_post_query->have_posts()) : $list_post_query->the_post();
	...
	$featured_image = '<a href="'.get_permalink().'" title="'.get_the_title().'" target="' . $link_target . '">'.get_the_post_thumbnail( $post->ID, array($featured_posts_image_width,$featured_posts_image_height),"class={$reflect}" ).'</a>';
	...
	ob_start(); ?>
		...
		<h3><a href="<?php the_permalink() ?>" title="<?php _e( 'Permanent Link to', 'cc' ) . the_title_attribute(); ?>" target="<?php echo $link_target; ?>"><?php the_title() ?></a></h3>
		...
		<p style="<?php $margintop ?> height:'<?php $height ?>'">
			...
			<?php if($hide_more_link == '') { ?>
				<a class="readmore" href="<?php the_permalink() ?>" target="<?php echo $link_target; ?>"><?php _e('read more','cc') ?></a>
			<?php } ?>
		</p>
