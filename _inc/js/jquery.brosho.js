...
(function(jQuery) {
  jQuery.fn.brosho = function(options) {
    var opts = jQuery.extend({}, jQuery.fn.brosho.defaults, options);
    
    
    if(jQuery('#brosho-wrapper').length) { //brosho got called the second time by user, remove the current and add back with the options
      jQuery('#brosho-wrapper, #brosho-overlay-wrapper').remove(); //remove old stuff
      jQuery('body *').unbind(); //unbind the previous hover event handler on every element within the body
    } else {
    ...
    jQuery('#brosho-position a', wrapper).click(function() { //position the editor as user requests it
      wrapper.removeClass().addClass('brosho-' + jQuery(this).text().toLowerCase()); //remove all classes on wrapper and add the one requested
    ...
    jQuery('#brosho-highlight a', wrapper).toggle(function() { //disable/enable the hover highlight on elements
      jQuery(this).text('Disabled'); //change text
      highlight = false; //no highlight
    }, function() {
      jQuery(this).text('Enabled'); //change text back
      highlight = true; //allow highlight again
    });
    

    jQuery('#brosho-generate a', wrapper).click(function() { //return the css code for the changed elements
      var full_css = ""; //store the generated css code here
         
      jQuery('body *').each(function() { //check every element for changed css
        var el = jQuery(this); //used several times
        var brosho_css = el.attr('brosho-css'); //store the attribute value
           
           
        if(brosho_css) { //does this element have changed css
          var temp_css = jQuery.fn.brosho.extractCssSelectorPath(el) + ' {\n' //generate css selector path
      ...
      jQuery('#brosho-css-output textarea', overlay).val(trim(full_css)).select(); //set the text for the css output textbox and autoselect it
      
      return false; //dont follow the anchor
    });
    
    
    jQuery('#brosho-css-output a').click(function() { //watch the close button on the css output overlay
      jQuery(this).parent().parent().fadeOut('fast');
      return false; //dont follow the anchor
    });
    
    
    var selector_field = jQuery('#brosho-selector-field', wrapper); //selector textbox, used multiple times
    var css_field = jQuery('#brosho-css-field', wrapper); //the css textarea, used multiple times
...
//printObject(el);
    ...
    jQuery('body *:not(#brosho-wrapper, #brosho-wrapper *, #brosho-overlay-wrapper, #brosho-overlay-wrapper *)').hover(function() {
      if(highlight) jQuery(this).addClass(opts.elementHoverClass); //add the hover class to the current element
    }, function() { //on mouseleave
      jQuery(this).removeClass(opts.elementHoverClass); //remove the hover class
    }).click(function() { //on element click
      var el = jQuery(this); //store the current element, used multiple times
      
      jQuery('body *').removeClass(opts.elementHoverClass); //remove the brosho hover class on every element so we dont generate a false path
      
      selector_field.val(jQuery.fn.brosho.extractCssSelectorPath(el)); //set the css selector path of the current element

//  alert(jQuery.fn.brosho.extractCssSelectorPath(el));
//	alert(el.attr('brosho-css'));
    ...
    css_field.keyup(function(event) { //watch key up's on the css editor
      if(css_field.val().substr(css_field.val().length - 1, 1) == ';' || //is the last typed character a ;
        ...
        jQuery(selector_field.val()).attr({ //set the new attributes on each matching element
    ...
    selector_field.blur(function() { //watch the blur event for the selector textbox
      var brosho_css = jQuery(selector_field.val()).attr('brosho-css'); //used multiple times
    ...
    wrapper.css('opacity', opts.editorOpacity).hover(function() { //make brosho semi transparent so you can see whats going on underneath
      jQuery(this).fadeTo('fast', 1); //fade to full
    }, function() {
      jQuery(this).fadeTo('fast', opts.editorOpacity); //fade back
    });
  }; //brosho function end
  ...
  jQuery.fn.brosho.extractCssSelectorPath = function(el) {
    if(el.attr('id')) return '#' + el.attr('id'); //it is (should) be an unique element, return the id selector
    
    var path = jQuery.fn.brosho.extractCssSelectorPath(el.parent()); //to prepend the path of the parent element
...
jQuery(function () { jQuery.fn.brosho(); }); //auto execute the brosho plugin
